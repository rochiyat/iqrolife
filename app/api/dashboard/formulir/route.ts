...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const response = await fetch(`${API_URL}/api/formulir`, {
        method: 'POST',
        headers,
    ...
    const data = await proxyToBackend(request, '/api/formulir', 'POST', body);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error submitting formulir:', error);
    return NextResponse.json({ error: 'Gagal mengirim formulir pendaftaran' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Gagal mengupdate formulir' }, { status: 500 });
  }
}
